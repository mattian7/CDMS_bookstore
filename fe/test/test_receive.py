...
class TestReceive:
    ...
    def test_non_exist_order(self):
        code = self.seller.send(self.seller_id, self.order_id)
        code = self.buyer.receive(self.buyer_id, self.password, self.order_id + 's')
        assert code != 200

    def test_false_buyer(self):
        code = self.seller.send(self.seller_id, self.order_id)
        code = self.buyer.receive(self.buyer_id+'s', self.password, self.order_id)
        assert code != 200
