...
class Buyer(db_conn.DBConn):
    ...
    def new_order(self, user_id: str, store_id: str, id_and_count: [(str, int)]) -> (int, str, str):
        ...
        for book_id, count in id_and_count:
            self.conn.execute("SELECT book_num from Books_in_store where store_id = '%s' and book_id = '%s'" %(store_id, book_id))
            row = self.conn.fetchone()
            if row is None:
    ...
    def cancel_order(self, user_id: str, order_id: str):
        try:
            ...
            self.conn.execute(
                "SELECT order_id, buyer, status FROM Orders WHERE order_id = '%s'" % (order_id))
            row = self.conn.fetchone()
            if row is None:
                return error.error_invalid_order_id(order_id)
